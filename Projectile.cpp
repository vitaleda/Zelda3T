...
Projectile::~Projectile() {
    ...
    if (suivant != NULL) {
        Projectile* proj_ = (Projectile*)suivant;
        suivant = NULL;
        delete proj_;
    }
}
...
void Projectile::draw(SDL_Surface* gpScreen) {
    ...
    if (suivant != NULL) if (((Projectile*)suivant)->vie == 0) delete (Projectile*)enleve(suivant);
...
void Projectile::move(int dx, int dy) {
    ...
    if ((id == 1 || id == 2 || id == 30 || id == 31 || id == 32) && max > 0) return;
...
Projectile* Projectile::getSuivant() {
    return (Projectile*)suivant;
}

int Projectile::getId() {return id;}
void Projectile::setVie(int v) {vie = v;}
